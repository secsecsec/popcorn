...
RESULT=`./bin/cp_ramdisk $RAMDISK_ADDR $RAMDISK_FILE 2>&1` 
if [ $? -ne 0 ]
then
  echo "ERROR TRACE: cp_ramdisk output:"
  echo "$RESULT"
  echo "$RESULT" >> $LOGFILE
  exit 1
fi
...
RESULT=`./bin/cmdline_tool $BOOT_ARGS 2>&1`
if [ $? -ne 0 ]
then
  echo "ERROR TRACE: bin/cmdline_tool output:"
  echo "$RESULT"
  echo "$RESULT" >> $LOGFILE
  _CMDLINE=`./bin/cmdline_tool`
  echo "$_CMDLINE"
